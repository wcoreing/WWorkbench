...
import { useWorkbenchCommand } from '../../workbench/useWorkbenchCommand'
...
export function SSHForwardPanel({ hosts, onStatus }: Props) {
  ...
  const [draftHostId, setDraftHostId] = useState<string | undefined>()
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      return p as SSHForwardPreset[]
    } catch (e) {
      onStatus((e as Error).message)
      return [] as SSHForwardPreset[]
    }
  }, [onStatus])
  ...
  useWorkbenchCommand(Capability.SSHForwardOpen, (cmd) => {
    const hostId = payloadStr(cmd.payload, 'hostId')
    const presetId = payloadStr(cmd.payload, 'presetId')
    const openNew = payloadBool(cmd.payload, 'openNew') || Boolean(hostId)
              ...
              {active.map((f) => (
                <li key={f.id} className="conn-item ssh-forward-item">
                  <div className="conn-meta">
                    ...
                    <span
                      className="conn-host"
                      title={t('sshForward.route', { local: f.localAddr, remote: `${f.remoteHost}:${f.remotePort}` })}
                    >
                      {f.localAddr} → {f.remoteHost}:{f.remotePort}
                    </span>
                  </div>
                  <div className="ssh-forward-actions">
                    <button type="button" className="wn-btn wn-btn-xs wn-btn-ghost" {...pressProps(() => copyAddr(f.localAddr))}>
                      {t('common.copy')}
                    </button>
                    <button
                      type="button"
                      ...
                      {...pressProps(() => void stopForward(f.id), { disabled: actingId === f.id })}
                    >
                      {t('sshForward.stop')}
                    </button>
                  </div>
          ...
          {presets.length === 0 ? (
            ...
            <ul className="conn-list ssh-forward-list">
              {presets.map((p) => (
                <li
                  key={p.id}
                  className="conn-item ssh-forward-item"
                  onDoubleClick={() => {
                    setEditing(p)
                    setDraftHostId(undefined)
                    setModalOpen(true)
                ...
                >
                  ...
                  <div className="ssh-forward-actions">
                    <button
                      ...
                      disabled={actingId === p.id || !hosts.some((h) => h.id === p.sshHostId)}
                      {...pressProps(() => void startPreset(p), {
                        disabled: actingId === p.id || !hosts.some((h) => h.id === p.sshHostId),
                      })}
                    >
                      {t('sshForward.start')}
                    </button>
                    <button type="button" className="wn-btn wn-btn-xs wn-btn-ghost" {...pressProps(() => setDeleteTarget(p))}>
                      {t('common.delete')}
                    </button>
                  </div>
                </li>

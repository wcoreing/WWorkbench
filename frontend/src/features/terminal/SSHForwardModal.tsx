...
export function SSHForwardModal({ open, hosts, initial, onClose, onSaved }: Props) {
  const { t } = useI18n()
        ...
        <div className="wn-modal-body">
          <div className="wn-form">
            <div className="wn-field">
              ...
              <input className="wn-input" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('sshForward.namePlaceholder')} autoFocus />
            </div>
            <div className="wn-field">
              ...
              <select className="wn-input" value={sshHostId} onChange={(e) => setSSHHostId(e.target.value)}>
                <option value="">{t('sshForward.pickHost')}</option>
                {hosts.map((h) => (
            ...
            <div className="wn-form-row">
              <div className="wn-field">
                ...
                <input className="wn-input" type="number" min={0} max={65535} value={localPort || ''} placeholder="0" onChange={(e) => setLocalPort(Number(e.target.value) || 0)} />
                <span className="wn-field-hint">{t('sshForward.localPortHint')}</span>
              </div>
              <div className="wn-field">
                <label className="wn-label">{t('sshForward.remotePort')}</label>
                <input className="wn-input" type="number" min={1} max={65535} value={remotePort} onChange={(e) => setRemotePort(Number(e.target.value) || 0)} />
              </div>
            </div>
            <div className="wn-field">
              <label className="wn-label">{t('sshForward.remoteHost')}</label>
              <input className="wn-input" value={remoteHost} onChange={(e) => setRemoteHost(e.target.value)} placeholder={t('sshForward.remoteHostPlaceholder')} />
            </div>
          </div>
          {error && <div className="wn-form-msg error">{error}</div>}
        </div>
        <footer className="wn-modal-footer">
          <button type="button" className="wn-btn wn-btn-tool" onClick={onClose} disabled={saving}>{t('common.cancel')}</button>
